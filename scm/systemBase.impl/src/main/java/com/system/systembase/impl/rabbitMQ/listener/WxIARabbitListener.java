...
@Slf4j
@Component
public class WxIARabbitListener {
     ...
     * @return void
     **/
    public void wxPushInventoryAlertMQ(InventoryAlertTemplateParam param) {
        log.info("wxPushMQ-参数-param: {}", JSON.toJSONString(param));
        try {
            rabbitTemplate.convertAndSend(RabbitEnum.WX_IA_DIRECT_EXCHANGE.getCode(), RabbitEnum.WX_IA_ROUTING_KEY.getCode(), param);
        } catch (Exception e) {
    ...
    /**
     *  预警库存-微信服务号消息模板推送监听事件
     * @author lutong
     * @data 2025-3-14 014
     * @param param
     * @return void
     **/
    @RabbitListener(queues = RabbitQueueConstant.WX_IA_QUEUE)
    public void wxPushInventoryAlertListener(InventoryAlertTemplateParam param) {
        // 发送邮件业务
        log.info("wxPushListener-预警库存-监听数据param: {}", JSON.toJSONString(param));
        String result = wxPushManager.pushInventoryAlert(param);
        if (result == null) {
